...
BluFi 在 ESP-IDF 版本更新中经历了迭代，所以在 ESP-IDF v3.3 和 IDF v4.x 等不同版本中，所需配置存在差异，详情可见下表：

|  配置使能（双模或单 BLE）  | ESP-IDF <br> Release/v3.3 |  ESP-IDF <br> Release/v4.1| ESP-IDF <br> Release/v4.2| ESP-IDF <br> Release/v4.3| ESP-IDF <br> Release/v4.4 |
|:----------- |:---------------------: | :---------------------:| :---------------------:|:---------------------: | :---------------------:|
...
| Nimble  BluFi  | ![alt text](../../../docs/_static/no-icon.png "不支持")  | ![alt text](../../../docs/_static/no-icon.png "不支持")  | ![alt text](../../../docs/_static/no-icon.png "不支持") | CONFIG_BT_ENABLED(=y) <br>  CONFIG_BT_BLUEDROID_ENABLED(=y) <br>  CONFIG_BT_NIMBLE_ENABLED(=y) <br>  CONFIG_BT_NIMBLE_BLUFI_ENABLE(=y) | CONFIG_BT_ENABLED(=y) <br>  CONFIG_BT_BLUEDROID_ENABLED(=y) <br>  CONFIG_BT_NIMBLE_ENABLED(=y) <br>  CONFIG_BT_NIMBLE_BLUFI_ENABLE(=y) |

**注意**
-  标签 ![alt text](../../../docs/_static/no-icon.png) 表示当前版本不支持。
...
- 您也可以选择重新配置 Wi-Fi 网络。长按 `SET` 键三秒以上，进入配网模式，此时开发板的绿灯会闪烁。接着使用乐鑫的 `EspBluFi` 蓝牙配网工具，搜索到名称为 `BLUFI_DEVICE` 的设备广播，点击 `连接`，然后点击 `配置网络`，`Blufi` 协议会自动配置 `Wi-Fi` 连接信息。成功后获取 IP 后，打印如下：
...
- 设备成功获取 IP 后，程序会使用内置的 `$ADF_PATH/components/dueros_service/duer_profile` 连接 DuerOS 服务器进行鉴权，DuerOS 连接成功后等待语音命令。打印如下：
