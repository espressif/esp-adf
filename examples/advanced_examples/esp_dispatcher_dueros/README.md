...
- After the example starts running, please try to connect to the preset Wi-Fi hotspot first. After successfully obtaining the IP, the example outputs the following log:
...
- After successfully obtaining the IP address, the program will use the built-in `$ADF_PATH/components/dueros_service/duer_profile` to connect to the DuerOS server for authentication. After successfully connected, DuerOS will wait for speech commands. The log is as follows:
