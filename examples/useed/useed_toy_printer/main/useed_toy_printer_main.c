/* Control with buttons playing MP3 files from SD Card
...
#define GPIO_INPUT_PIN_SEL  ( (1ULL<<BUTTON_NEXT) | (1ULL<<BUTTON_PLAY) )
...
static esp_err_t cli_next(esp_periph_handle_t periph, int argc, char *argv[])
{
...
const periph_console_cmd_t cli_cmd[] = {
    ...
    { .cmd = "next",        .id = 2, .help = "Next song",				.func = cli_next },
};
    ...
    while (1) {
        ...
        */
        ...
        if (msg.source_type == AUDIO_ELEMENT_TYPE_ELEMENT){
            ...
            if (msg.source == (void *) mp3_decoder
                ...
                ESP_LOGI(TAG, "[ * ] Received music info from mp3 decoder, sample_rates=%d, bits=%d, ch=%d, volume=%d, codec_fmt=%d",
                    music_info.sample_rates, music_info.bits, music_info.channels, music_info.volume, music_info.codec_fmt);
        ...
        */
        if (msg.source_type == PERIPH_ID_BUTTON
            ...
            } else if ((int) msg.data == BUTTON_NEXT) {
                ESP_LOGI(TAG, "[ * ] [Set] touch tap event");
                ESP_LOGI(TAG, "[ * ] Finishing current song");
                // Stop the current song playing
                audio_pipeline_stop(pipeline);
            ...
            /*
            else if ((int) msg.data == BUTTON_VOLUP) {
            ...
            } else if ((int) msg.data == BUTTON_VOLDWN) {
                ESP_LOGI(TAG, "[ * ] [Vol-] touch tap event");
                player_volume -= 10;
                if (player_volume < 0) {
                    player_volume = 0;
