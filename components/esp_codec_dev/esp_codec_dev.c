...
int esp_codec_dev_read_reg(esp_codec_dev_handle_t handle, int reg, int *val)
{
    ...
    if (dev && dev->codec_if && dev->codec_if->get_reg) {
        dev->codec_if->get_reg(dev->codec_if, reg, val);
    }
    return ESP_CODEC_DEV_OK;
}
...
int esp_codec_dev_write_reg(esp_codec_dev_handle_t handle, int reg, int val)
{
    ...
    if (dev && dev->codec_if && dev->codec_if->set_reg) {
        dev->codec_if->set_reg(dev->codec_if, reg, val);
    }
    return ESP_CODEC_DEV_OK;
}
