...
{
    uint8_t reg_low = reg & 0xFE;   // bit0=0
    uint8_t reg_high = reg | 0x01;  // bit0=1
    uint8_t data_low = 0, data_high = 0;
    
    int ret = codec->cfg.ctrl_if->read_reg(codec->cfg.ctrl_if, reg_low, 1, &data_low, 1);
    if (ret == 0) {
        ret = codec->cfg.ctrl_if->read_reg(codec->cfg.ctrl_if, reg_high, 1, &data_high, 1);
    }
    
    *value = data_low | ((data_high & 0x01) << 8);
    ESP_LOGD(TAG, "Read reg %d, value %d", reg, *value);
    return ret;
}
...
const cjc8910_reg_map_t reg_map[] = {
    {CJC8910_R0_LEFT_INPUT_VOLUME,      "R0",  "Left Input Volume"},
    {CJC8910_R2_LOUT1_VOLUME,           "R2",  "Left Out1 Volume"},
    {CJC8910_R5_ADC_DAC_CONTROL,        "R5",  "ADC DAC Control"},
    {CJC8910_R7_AUDIO_INTERFACE,        "R7",  "Audio Interface"},
    {CJC8910_R8_SAMPLE_RATE,            "R8",  "Sample Rate"},
    {CJC8910_R10_LEFT_DAC_VOLUME,       "R10", "Left DAC Volume"},
    {CJC8910_R12_BASS_CONTROL,          "R12", "Bass Control"},
    {CJC8910_R13_TREBLE_CONTROL,        "R13", "Treble Control"},
    {CJC8910_R15_RESET,                 "R15", "Reset"},
    {CJC8910_R17_ALC1_CONTROL,          "R17", "ALC1 Control"},
    {CJC8910_R18_ALC2_CONTROL,          "R18", "ALC2 Control"},
    {CJC8910_R19_ALC3_CONTROL,          "R19", "ALC3 Control"},
    {CJC8910_R20_NOISE_GATE_CONTROL,    "R20", "Noise Gate Control"},
    {CJC8910_R21_LEFT_ADC_VOLUME,       "R21", "Left ADC Volume"},
    {CJC8910_R23_ADDITIONAL_CONTROL1,   "R23", "Additional Control 1"},
    {CJC8910_R24_ADDITIONAL_CONTROL2,   "R24", "Additional Control 2"},
    {CJC8910_R25_PWR_MGMT1_L,           "R25L", "Power Management 1 Low"},
    {CJC8910_R25_PWR_MGMT1_H,           "R25H", "Power Management 1 High"},
    {CJC8910_R26_PWR_MGMT2_L,           "R26L", "Power Management 2 Low"},
    {CJC8910_R26_PWR_MGMT2_H,           "R26H", "Power Management 2 High"},
    {CJC8910_R27_ADDITIONAL_CONTROL3,   "R27", "Additional Control 3"},
    {CJC8910_R32_ADCL_SIGNAL_PATH,      "R32", "ADCL Signal Path"},
    {CJC8910_R33_MIC,                   "R33", "MIC Control"},
    {CJC8910_R34_AUX,                   "R34", "AUX Control"},
    {CJC8910_R35_LEFT_OUT_MIX2_L,       "R35L", "Left Out Mix2 Low"},
    {CJC8910_R35_LEFT_OUT_MIX2_H,       "R35H", "Left Out Mix2 High"},
    {CJC8910_R37_ADC_PDN,               "R37", "ADC Power Down"},
    {CJC8910_R67_LOW_POWER_PLAYBACK,    "R67", "Low Power Playback"},
};
...
static void cjc8910_dump(const audio_codec_if_t *h){
    ...
    ESP_LOGI(TAG, "========== CJC8910 Register Dump ==========");
    const int reg_count = sizeof(reg_map) / sizeof(reg_map[0]);
        for (int i = 0; i < reg_count; i++) {
        int data =0;
        cjc8910_read_reg(codec, reg_map[i].reg_addr, &data);
        ESP_LOGI(TAG, "%-6s (0x%02X): 0x%03X - %s", 
                 reg_map[i].reg_name, 
                 reg_map[i].reg_addr, 
                 data, 
                 reg_map[i].description);
    }
...
static int cjc8910_set_vol(const audio_codec_if_t *h, float db_value){
    ...
    ESP_LOGD(TAG, "========== Set cjc8910 vol: %f ==========", db_value);
    return ret;
}
...
static int cjc8910_set_mic_gain(const audio_codec_if_t *h, float db){
    ...
    int ret = 0;
    uint8_t volume_reg_value = 0;
    if (db < -97.0) {
        volume_reg_value = 0x00;
    } else if (db >= 30.0) {
        volume_reg_value = 0xFF;
    } else {
        volume_reg_value= (uint8_t) (db * 2.0 + 195.0);
    }
    ret |= cjc8910_write_reg(codec, CJC8910_R21_LEFT_ADC_VOLUME, volume_reg_value);
    ESP_LOGD(TAG, "========== Set cjc8910 mic gain: %f, reg: %d ==========", db, volume_reg_value);
    return ret;
}
...
static int  cjc8910_set_bits_per_sample(audio_codec_cjc8910_t *codec, int bits)
{
    ...
    ESP_LOGE(TAG,"%s:set bits_per_sample:%d, audio_iface:%d", __func__, bits, audio_iface);
...
static int cjc8910_set_fs(const audio_codec_if_t *h, esp_codec_dev_sample_info_t *fs)
{
    ...
    ESP_LOGI(TAG, "========== Set cjc8910 sample_rate:%ld, channel:%d, bits_per_sample:%d ==========", fs->sample_rate, fs->channel, fs->bits_per_sample);
    return ret;
}
