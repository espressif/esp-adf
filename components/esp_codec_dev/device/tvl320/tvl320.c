...
static int tvl320_open(const audio_codec_if_t *h, void *cfg, int cfg_size)
{
    ...
    ret |= tvl320_write_reg(codec, TVL320_PAGE_0_CONTROL_REG00, TVL320_PAGE_0);

    // Initiate SW Reset
    ret |= tvl320_write_reg(codec, TVL320_SOFTWARE_RESET_REG01, 0x01);

    // Program and power up NDAC
    ret |= tvl320_write_reg(codec, TVL320_DAC_NDAC_VAL_REG11, 0x88);

    // Program and power up MDAC
    ret |= tvl320_write_reg(codec, TVL320_DAC_MDAC_VAL_REG12, 0x82);

    // Program OSR value
    // DOSR = 128, DOSR(9:8) = 0, DOSR(7:0) = 128
    ret |= tvl320_write_reg(codec, TVL320_DAC_DOSR_VAL_MSB_REG13, 0x00);
    ret |= tvl320_write_reg(codec, TVL320_DAC_DOSR_VAL_LSB_REG14, 0x80);

    // mode is i2s, wordlength is 16, slave mode
    ret |= tvl320_write_reg(codec, TVL320_CODEC_INTERFACE_CONTROL_REG27, 00);

    // Program the processing block to be used
    // Select DAC DSP Processing Block PRB_P16
    ret |= tvl320_write_reg(codec, TVL320_DAC_PROCESSING_BLOCK_MINIDSP_SELECTION_REG60, 0x10);
    ret |= tvl320_write_reg(codec, TVL320_PAGE_0_CONTROL_REG00, TVL320_PAGE_8);
    ret |= tvl320_write_reg(codec, TVL320_DAC_COEFFICIENT_RAM_CONTROL_REG01, 0x04);
    ret |= tvl320_write_reg(codec, TVL320_PAGE_8_CONTROL_REG00, TVL320_PAGE_0);

    // Program analog blocks
    // Page 1 is selected
    ret |= tvl320_write_reg(codec, TVL320_PAGE_8_CONTROL_REG00, TVL320_PAGE_1);

    // Program common-mode voltage (default = 1.35V)
    ret |= tvl320_write_reg(codec, TVL320_HEADPHONE_DRIVERS_REG31, 0x04);

    // Program headphone-specific depop settings (in case headphone driver is used)
    // De-pop, Power on = 800 ms, Step time = 4 ms
    ret |= tvl320_write_reg(codec, TVL320_HP_OUTPUT_DRIVERS_POP_REMOVAL_SETTINGS_REG33, 0x4e);

    // Program routing of DAC output ot the output amplifier (headphone/lineout or speaker)
    // DAC routed to HPOUT
    ret |= tvl320_write_reg(codec, TVL320_DAC_OUTPUT_MIXER_ROUTING_REG35, 0x40);

    // Unmute and set gain of output driver
    // Unmute HPOUT, set gain = 0db
    ret |= tvl320_write_reg(codec, TVL320_HPOUT_DRIVER_REG40, 0x06);

    // Unmute Class-D, set gain = 18 db
    ret |= tvl320_write_reg(codec, TVL320_CLASS_D_OUTPUT_DRIVER_DRIVER_REG42, 0x1C);

    // HPOUT powered up
    ret |= tvl320_write_reg(codec, TVL320_HEADPHONE_DRIVERS_REG31, 0x82);

    // Power-up Class-D drivers
    ret |= tvl320_write_reg(codec, TVL320_CLASS_D_SPEAKER_AMP_REG32, 0xC6);

    // Enable HPOUT output analog volume, set = -9 dB
    ret |= tvl320_write_reg(codec, TVL320_ANALOG_VOLUME_TO_HPOUT_REG36, 0x92);

    // Enable Class-D output analog volume, set = -9 dB
    ret |= tvl320_write_reg(codec, TVL320_ANALOG_VOLUME_TO_CLASS_D_OUTPUT_DRIVER_REG38, 0x92);

    // Page 0 is selected
    ret |= tvl320_write_reg(codec, TVL320_PAGE_1_CONTROL_REG00, TVL320_PAGE_0);

    // Powerup DAC (soft step disable)
    ret |= tvl320_write_reg(codec, TVL320_DAC_DATA_PATH_SETUP_REG63, 0x94);

    // DAC gain = -22 dB
    ret |= tvl320_write_reg(codec, TVL320_DAC_VOLUME_CONTROL_REG65, 0xD4);

    // Unmute digital volume control
    // Unmute DAC
    ret |= tvl320_write_reg(codec, TVL320_DAC_MUTE_CONTROL_REG64, 0x04);
...
static int tvl320_set_vol(const audio_codec_if_t *h, float db_value)
{
    ...
    int ret = tvl320_write_reg(codec, TVL320_DAC_VOLUME_CONTROL_REG65, volume);
...
static int tvl320_set_mute(const audio_codec_if_t *h, bool mute)
{
    ...
    return tvl320_write_reg(codec, TVL320_DAC_MUTE_CONTROL_REG64, (uint8_t) regv);
}
...
static int tvl320_set_reg(const audio_codec_if_t *h, int reg, int value)
{
    ...
    return tvl320_write_reg(codec, reg, value);
}
