...
{
    (void)src;
    static esp_capture_format_id_t supstream_codecs[] = {ESP_CAPTURE_FMT_ID_PCM};
    *codecs = supstream_codecs;
    *num = 1;
    return ESP_CAPTURE_ERR_OK;
}
...
static esp_capture_err_t fake_aud_src_read_frame(esp_capture_audio_src_if_t *h, esp_capture_stream_frame_t *frame)
{
    ...
    if (src->info.bits_per_sample == 16) {
        ...
        int v = src->frame_num % 65536;
        for (int i = 0; i < samples; i++) {
            int16_t wav_value = (int16_t)(v > 32768 ? 65536 - v : v);
