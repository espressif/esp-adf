...
static const char *TAG = "APM_LINK";

static const esp_media_stream_id_t s_src_stream = ESP_MEDIA_DEFAULT_STREAM;
static const esp_media_stream_id_t s_player_stream = (esp_media_stream_id_t)1;
...
esp_err_t link_source_start(void)
{
    ...
    /* Dummy SRC = looping pattern AAC. Replace create/add_track with your SRC. */
    ...
    ret = esp_media_dummy_service_add_track(s_src, s_src_stream, &audio);
    if (ret != ESP_OK) {
        goto fail;
    }

    ret = esp_media_service_link(ESP_SERVICE_BASE(s_src), s_src_stream,
                                 ESP_SERVICE_BASE(s_player), s_player_stream);
    if (ret != ESP_OK) {
        goto fail;
    }
...
esp_err_t link_source_stop(void)
{
    ...
    if (s_player != NULL) {
        (void)esp_media_service_unlink(ESP_SERVICE_BASE(s_src), s_src_stream,
                                       ESP_SERVICE_BASE(s_player), s_player_stream);
    }
