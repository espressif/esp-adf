...
static const char *TAG = "AUD_CAP_MCP";

extern const uint8_t esp_audio_capture_service_mcp_json_start[] asm("_binary_esp_audio_capture_service_mcp_json_start");
...
esp_err_t esp_audio_capture_service_mcp_schema_get(const char **out_schema)
{
    ...
    *out_schema = (const char *)esp_audio_capture_service_mcp_json_start;
    return ESP_OK;
}
...
static esp_err_t tool_apply_setup(esp_capture_service_t *capture, const char *args, char *result, size_t result_size)
{
    ...
    if (stream_num <= 0 || stream_num > ESP_AUDIO_CAPTURE_SERVICE_MAX_STREAM_NUM) {
        cJSON_Delete(root);
        return ESP_ERR_INVALID_ARG;
    }

    esp_audio_capture_service_setup_t setup = {0};
    ...
    for (int i = 0; i < stream_num; i++) {
        ...
        esp_audio_capture_service_stream_cfg_t *cfg = &setup.streams[i];
        if (!json_get_optional_bool(stream, "enabled", true, &cfg->enabled)) {
            cJSON_Delete(root);
            return ESP_ERR_INVALID_ARG;
        }
        const cJSON *codec_item = cJSON_GetObjectItemCaseSensitive(stream, "codec");
        uint32_t codec = 0;
        if (!parse_fourcc(codec_item, &codec)) {
            cJSON_Delete(root);
            return ESP_ERR_INVALID_ARG;
        }
        cfg->audio_info.codec = codec;
        uint32_t sample_rate = 0;
        uint32_t bitrate = 0;
        uint16_t bits = 0;
        uint16_t channel = 0;
        if (!json_get_optional_uint32(stream, "sample_rate", 16000, &sample_rate) ||
            !json_get_optional_uint16(stream, "bits_per_sample", 16, &bits) ||
            !json_get_optional_uint16(stream, "channel", 1, &channel) ||
            !json_get_optional_uint32(stream, "bitrate", 0, &bitrate)) {
            cJSON_Delete(root);
            return ESP_ERR_INVALID_ARG;
        }
        ...
        cfg->audio_info.bitrate = bitrate;
        ...
        if (muxer_item) {
            ...
            cfg->muxer.muxer_type = (esp_muxer_type_t)muxer_type;
        }
        if (!json_get_optional_bool(stream, "auto_record", false, &cfg->muxer.auto_record) ||
            !json_get_optional_bool(stream, "streaming", false, &cfg->muxer.streaming) ||
            !json_get_optional_string(stream, "storage_dir", &cfg->muxer.storage_dir)) {
            cJSON_Delete(root);
            return ESP_ERR_INVALID_ARG;
        }
    }

    esp_err_t op_ret = esp_audio_capture_service_apply_setup(capture, &setup);
    ret = finish_op(result, result_size, "esp_audio_capture_service_apply_setup", op_ret);
    cJSON_Delete(root);
    return ret;
}
...
static esp_err_t tool_enable_stream(esp_capture_service_t *capture, const char *args, char *result, size_t result_size)
{
    ...
    ret = finish_op(result, result_size, "esp_audio_capture_service_enable_stream", op_ret);
    cJSON_Delete(root);
    return ret;
}
...
static esp_err_t tool_record(esp_capture_service_t *capture, bool start, const char *args, char *result, size_t result_size)
{
    ...
    ret = finish_op(result, result_size,
                    start ? "esp_audio_capture_service_start_record" : "esp_audio_capture_service_stop_record",
                    op_ret);
    cJSON_Delete(root);
    return ret;
}
...
static esp_err_t tool_set_storage_url(esp_capture_service_t *capture, const char *args, char *result, size_t result_size)
{
    ...
    ret = finish_op(result, result_size, "esp_audio_capture_service_set_storage_url", op_ret);
    cJSON_Delete(root);
    return ret;
}
                                           ...
                                           size_t result_size)
{
    ...
    if (op_ret != ESP_OK) {
        ret = finish_op(result, result_size, "esp_audio_capture_service_get_last_storage_url", op_ret);
        cJSON_Delete(root);
        return ret;
    }
    ...
    if (!out ||
        !cJSON_AddStringToObject(out, "operation", "esp_audio_capture_service_get_last_storage_url") ||
                                            ...
                                            size_t result_size)
{
    ...
    if (resolved == NULL) {
        ...
        if (url_ret != ESP_OK) {
            ret = finish_op(result, result_size, "esp_audio_capture_service_get_storage_file_info", url_ret);
            cJSON_Delete(root);
            return ret;
        }
    }
    ...
    if (op_ret != ESP_OK) {
        ret = finish_op(result, result_size, "esp_audio_capture_service_get_storage_file_info", op_ret);
        cJSON_Delete(root);
        return ret;
    }
    ...
    esp_err_t json_ret = ESP_OK;
    if (!out ||
        !cJSON_AddStringToObject(out, "operation", "esp_audio_capture_service_get_storage_file_info") ||
        !cJSON_AddBoolToObject(out, "ok", true) ||
        !cJSON_AddNumberToObject(out, "stream", stream) ||
...
static esp_err_t tool_get_status(esp_capture_service_t *capture, char *result, size_t result_size)
{
    ...
    esp_err_t json_ret = ESP_OK;
    if (!cJSON_AddStringToObject(out, "operation", "esp_audio_capture_service_get_status") ||
        !cJSON_AddBoolToObject(out, "ok", true) ||
                                                ...
                                                const char *args, char *result, size_t result_size)
{
    ...
    if (strcmp(tool->name, "esp_audio_capture_service_apply_setup") == 0) {
        return tool_apply_setup(capture, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_audio_capture_service_start") == 0) {
        return finish_op(result, result_size, tool->name, esp_service_start(ESP_SERVICE_BASE(capture)));
    }
    if (strcmp(tool->name, "esp_audio_capture_service_stop") == 0) {
        return finish_op(result, result_size, tool->name, esp_service_stop(ESP_SERVICE_BASE(capture)));
    }
    if (strcmp(tool->name, "esp_audio_capture_service_enable_stream") == 0) {
        return tool_enable_stream(capture, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_audio_capture_service_start_record") == 0) {
        return tool_record(capture, true, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_audio_capture_service_stop_record") == 0) {
        return tool_record(capture, false, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_audio_capture_service_set_storage_url") == 0) {
        return tool_set_storage_url(capture, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_audio_capture_service_get_last_storage_url") == 0) {
        return tool_get_last_storage_url(capture, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_audio_capture_service_get_storage_file_info") == 0) {
        return tool_get_storage_file_info(capture, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_audio_capture_service_get_status") == 0) {
