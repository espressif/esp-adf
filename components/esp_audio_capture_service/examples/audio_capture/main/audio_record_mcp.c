...
static const char *TAG = "AUDIO_REC_MCP";

#if CONFIG_ESP_MCP_TRANSPORT_UART
#ifndef CONFIG_AUDIO_RECORD_MCP_UART_PORT
#define CONFIG_AUDIO_RECORD_MCP_UART_PORT  1
#endif  /* CONFIG_AUDIO_RECORD_MCP_UART_PORT */
#ifndef CONFIG_AUDIO_RECORD_MCP_UART_TX_IO
#define CONFIG_AUDIO_RECORD_MCP_UART_TX_IO  21
#endif  /* CONFIG_AUDIO_RECORD_MCP_UART_TX_IO */
#ifndef CONFIG_AUDIO_RECORD_MCP_UART_RX_IO
#define CONFIG_AUDIO_RECORD_MCP_UART_RX_IO  22
#endif  /* CONFIG_AUDIO_RECORD_MCP_UART_RX_IO */
#ifndef CONFIG_AUDIO_RECORD_MCP_UART_BAUD
#define CONFIG_AUDIO_RECORD_MCP_UART_BAUD  115200
#endif  /* CONFIG_AUDIO_RECORD_MCP_UART_BAUD */
#endif  /* CONFIG_ESP_MCP_TRANSPORT_UART */
...
esp_err_t audio_record_mcp_start(void)
{
    ...
    ESP_RETURN_ON_ERROR(esp_audio_capture_service_create(&service_cfg, &s_capture), TAG, "Create capture failed");
    ...
    ESP_RETURN_ON_ERROR(esp_audio_capture_service_apply_setup(s_capture, &setup), TAG, "Apply setup failed");
    ...
    const char *audio_schema = NULL;
    ESP_RETURN_ON_ERROR(esp_audio_capture_service_mcp_schema_get(&audio_schema), TAG, "Get audio schema failed");
    esp_service_registration_t audio_reg = {
        .service = ESP_SERVICE_BASE(s_capture),
        .category = "audio",
        .flags = ESP_SERVICE_REG_FLAG_SKIP_BATCH_START | ESP_SERVICE_REG_FLAG_SKIP_BATCH_STOP,
        .tool_desc = audio_schema,
        .tool_invoke = esp_audio_capture_service_tool_invoke,
    };
    ESP_RETURN_ON_ERROR(esp_service_manager_register(mgr, &audio_reg), TAG, "Register audio MCP failed");
...
#endif  /* CONFIG_ESP_MEDIA_SERVICE_MCP_ENABLE */
    ESP_LOGI(TAG, "Audio capture + dummy sink MCP tools registered");

#if CONFIG_ESP_MCP_TRANSPORT_UART
    esp_service_mcp_trans_t *transport = NULL;
    esp_service_mcp_uart_config_t uart_cfg = ESP_SERVICE_MCP_UART_CONFIG_DEFAULT();
    uart_cfg.uart_port = CONFIG_AUDIO_RECORD_MCP_UART_PORT;
    uart_cfg.tx_pin = CONFIG_AUDIO_RECORD_MCP_UART_TX_IO;
    uart_cfg.rx_pin = CONFIG_AUDIO_RECORD_MCP_UART_RX_IO;
    uart_cfg.baud_rate = CONFIG_AUDIO_RECORD_MCP_UART_BAUD;
    ...
    server_cfg.server_name = "audio-record-example";
    ...
    ESP_LOGI(TAG, "MCP UART started: port=%d tx=%d rx=%d baud=%d",
             CONFIG_AUDIO_RECORD_MCP_UART_PORT, CONFIG_AUDIO_RECORD_MCP_UART_TX_IO,
             CONFIG_AUDIO_RECORD_MCP_UART_RX_IO, CONFIG_AUDIO_RECORD_MCP_UART_BAUD);
#else
...
#else  /* !CONFIG_ESP_AUDIO_CAPTURE_SERVICE_MCP_ENABLE */
