...
"""
Audio capture service MCP UART test client.
...
Usage:
    python3 scripts/test_audio_capture_mcp_uart.py <serial-port> [baud-rate]
...
EXPECTED_TOOLS = {
    'esp_audio_capture_service_apply_setup',
    'esp_audio_capture_service_start',
    'esp_audio_capture_service_stop',
    'esp_audio_capture_service_enable_stream',
    'esp_audio_capture_service_start_record',
    'esp_audio_capture_service_stop_record',
    'esp_audio_capture_service_set_storage_url',
    'esp_audio_capture_service_get_last_storage_url',
    'esp_audio_capture_service_get_storage_file_info',
    'esp_audio_capture_service_get_status',
...
def run_tests(client, run_seconds):
    ...
    print('\n== Test: esp_audio_capture_service_get_status ==')
    status, _ = parse_tool_text_result(
        client.call_tool('esp_audio_capture_service_get_status'),
        'esp_audio_capture_service_get_status',
    )
    print_json('Status:', status)

    print('\n== Test: esp_audio_capture_service_apply_setup ==')
    setup, _ = parse_tool_text_result(
        client.call_tool('esp_audio_capture_service_apply_setup', {
    ...
    cap_start, _ = parse_tool_text_result(
        client.call_tool('esp_audio_capture_service_start'),
        'esp_audio_capture_service_start',
    )
    ...
    cap_stop, _ = parse_tool_text_result(
        client.call_tool('esp_audio_capture_service_stop'),
        'esp_audio_capture_service_stop',
    )
    ...
    if not isinstance(stats, dict) or int(stats.get('audio_frame_count', 0)) <= 0:
        raise AssertionError(f'Expected audio_frame_count > 0, got {stats}')
    ...
    storage_url = '/sdcard/audio_record/mcp_audio.mp4'
    print('\n== Test: re-setup with muxer for storage / control tools ==')
    setup_storage, _ = parse_tool_text_result(
        ...
        'esp_audio_capture_service_apply_setup (storage)',
    )
    expect_ok(setup_storage, 'apply_setup storage')
    print_json('Apply setup (storage):', setup_storage)

    set_url, _ = parse_tool_text_result(
        client.call_tool('esp_audio_capture_service_set_storage_url', {
            'stream': 0,
            'url': storage_url,
        }),
        'esp_audio_capture_service_set_storage_url',
    ...
    print('\n== Test: start capture for record / enable_stream ==')
    cap_start2, _ = parse_tool_text_result(
        client.call_tool('esp_audio_capture_service_start'),
        'esp_audio_capture_service_start (storage)',
    )
    expect_ok(cap_start2, 'capture start storage')

    enable_off, _ = parse_tool_text_result(
        client.call_tool('esp_audio_capture_service_enable_stream', {'stream': 0, 'enable': False}),
        'esp_audio_capture_service_enable_stream disable',
    )
    expect_ok(enable_off, 'enable_stream disable')
    print_json('Disable stream:', enable_off)

    enable_on, _ = parse_tool_text_result(
        client.call_tool('esp_audio_capture_service_enable_stream', {'stream': 0, 'enable': True}),
        'esp_audio_capture_service_enable_stream enable',
    )
    expect_ok(enable_on, 'enable_stream enable')
    print_json('Enable stream:', enable_on)

    start_rec, _ = parse_tool_text_result(
        client.call_tool('esp_audio_capture_service_start_record', {'stream': 0}),
        'esp_audio_capture_service_start_record',
    )
    ...
    stop_rec, _ = parse_tool_text_result(
        client.call_tool('esp_audio_capture_service_stop_record', {'stream': 0}),
        'esp_audio_capture_service_stop_record',
    )
    ...
    last_url, _ = parse_tool_text_result(
        client.call_tool('esp_audio_capture_service_get_last_storage_url', {'stream': 0}),
        'esp_audio_capture_service_get_last_storage_url',
    )
    ...
    file_info, _ = parse_tool_text_result(
        client.call_tool('esp_audio_capture_service_get_storage_file_info', {'stream': 0}),
        'esp_audio_capture_service_get_storage_file_info',
    )
    ...
    cap_stop2, _ = parse_tool_text_result(
        client.call_tool('esp_audio_capture_service_stop'),
        'esp_audio_capture_service_stop (storage)',
    )
    expect_ok(cap_stop2, 'capture stop storage')
    print_json('Stop capture after record:', cap_stop2)

    print('\nAll audio capture MCP UART checks passed')


def main():
    parser = argparse.ArgumentParser(description='Test audio capture MCP tools over UART')
    ...
    print('=' * 72)
    print('  Audio Capture Service MCP UART Test')
    print('=' * 72)
