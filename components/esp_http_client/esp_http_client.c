...
int esp_http_client_read(esp_http_client_handle_t client, char *buffer, int len)
{
    ...
    while (need_read > 0 && is_data_remain) {
        ...
        rlen = esp_transport_read(client->transport, res_buffer->data, byte_to_read, HTTP_STREAM_READ_TIMEOUT_MS);
...
esp_err_t esp_http_client_open(esp_http_client_handle_t client, int write_len)
{
    ...
    if ((err = esp_http_client_set_post_field(client, NULL, write_len)) != ESP_OK) {
        return err;
    }
    if ((err = esp_http_client_request_send(client)) != ESP_OK) {
        return err;
