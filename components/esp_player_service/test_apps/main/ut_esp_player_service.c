                                       ...
                                       void *writer_ctx)
{
    ...
    return esp_player_service_apply_setup(service, &setup_cfg);
}

void esp_player_service_ut_force_link(void)  { }

TEST_CASE("player service get_role is invalid before setup", "[esp_player_service]")
{
    esp_player_service_cfg_t cfg = ESP_PLAYER_SERVICE_CFG_DEFAULT();
    esp_player_service_t *service = NULL;
    TEST_ASSERT_EQUAL(ESP_OK, esp_player_service_create(&cfg, &service));
...
TEST_CASE("esp_player_service creates and accepts audio tracks", "[esp_player_service]")
{
    esp_player_service_cfg_t cfg = ESP_PLAYER_SERVICE_CFG_DEFAULT();
    ...
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG,
                      esp_player_service_get_stream_info(service, info.max_stream_num, &stream_info));
    TEST_ASSERT_EQUAL(ESP_OK, esp_player_service_get_stream_info(service, 0, &stream_info));
    TEST_ASSERT_NULL(stream_info.mixer_stream);
...
TEST_CASE("player service mix cfg validates gains and preempt policy", "[esp_player_service]")
{
    esp_player_service_cfg_t cfg = ESP_PLAYER_SERVICE_CFG_DEFAULT();
    cfg.max_stream_num = 2;
    esp_player_service_t *service = NULL;
...
TEST_CASE("player service volume APIs for output and per-stream ALC", "[esp_player_service]")
{
    esp_player_service_cfg_t cfg = ESP_PLAYER_SERVICE_CFG_DEFAULT();
    cfg.max_stream_num = 2;
    esp_player_service_t *service = NULL;
    ...
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, esp_player_service_set_sync_mode(NULL, stream0, ESP_PLAYER_SYNC_MODE_AUDIO));
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, esp_player_service_set_sync_mode(service, 2, ESP_PLAYER_SYNC_MODE_AUDIO));
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, esp_player_service_set_sync_mode(service, stream0, ESP_PLAYER_SYNC_MODE_MAX));
    TEST_ASSERT_EQUAL(ESP_OK, esp_player_service_set_sync_mode(service, stream0, ESP_PLAYER_SYNC_MODE_NONE));
    TEST_ASSERT_EQUAL(ESP_OK, esp_player_service_set_sync_mode(service, stream1, ESP_PLAYER_SYNC_MODE_AUDIO));
...
TEST_CASE("esp_player_service setup can be called multiple times before start", "[esp_player_service]")
{
    esp_player_service_cfg_t cfg = ESP_PLAYER_SERVICE_CFG_DEFAULT();
    esp_player_service_t *service = NULL;
    TEST_ASSERT_EQUAL(ESP_OK, esp_player_service_create(&cfg, &service));

    esp_player_service_setup_t first_cfg = ESP_PLAYER_SERVICE_SETUP_DEFAULT();
    TEST_ASSERT_EQUAL(ESP_OK, esp_player_service_apply_setup(service, &first_cfg));
    ...
    TEST_ASSERT_EQUAL(ESP_OK, esp_player_service_set_output_volume(service, 80));
    esp_player_service_setup_t second_cfg = ESP_PLAYER_SERVICE_SETUP_DEFAULT();
    TEST_ASSERT_EQUAL(ESP_OK, esp_player_service_apply_setup(service, &second_cfg));
    TEST_ASSERT_EQUAL(ESP_OK, esp_player_service_get_output_volume(service, &volume));
    TEST_ASSERT_EQUAL(80, volume);

    TEST_ASSERT_EQUAL(ESP_OK, esp_service_start(ESP_SERVICE_BASE(service)));
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_STATE,
                      esp_player_service_apply_setup(service, &second_cfg));
    TEST_ASSERT_EQUAL(ESP_OK, esp_service_stop(ESP_SERVICE_BASE(service)));
    audio_ut_destroy_service(service);
}

TEST_CASE("esp_player_service links from media source provider", "[esp_player_service]")
{
    ...
    esp_player_service_setup_t setup_cfg = ESP_PLAYER_SERVICE_SETUP_DEFAULT();
    TEST_ASSERT_EQUAL(ESP_OK, esp_player_service_apply_setup(sink, &setup_cfg));
...
TEST_CASE("esp_player_service - custom instance with 100 PCM writes", "[esp_player_service]")
{
...
TEST_CASE("player service mixes two concurrent streams", "[esp_player_service]")
{
    ...
    /* Both streams should have opened a render stream and mixed output reached the writer. */
...
TEST_CASE("player service link feeds frames from provider to render", "[esp_player_service]")
{
...
TEST_CASE("player service playlist binding and error semantics", "[esp_player_service]")
{
    esp_player_service_cfg_t cfg = ESP_PLAYER_SERVICE_CFG_DEFAULT();
    esp_player_service_t *service = NULL;
    TEST_ASSERT_EQUAL(ESP_OK, esp_player_service_create(&cfg, &service));
...
TEST_CASE("player service playlist navigation drives playback", "[esp_player_service]")
{
    audio_ut_capture_ctx_t cap = {0};
...
TEST_CASE("player service link sink-stop-first wakes bridge task", "[esp_player_service]")
{
    /* Source-stop-first is already covered elsewhere (write_abort then stop).
       ...
       so esp_service_stop returns ESP_OK. */
    esp_media_track_info_t track = make_audio_track(1);
    mock_source_t source;
    init_source(&source, &track, 1);
    ...
    /* Sink stops first; must join the bridge task. */
    TEST_ASSERT_EQUAL(ESP_OK, esp_service_stop(ESP_SERVICE_BASE(sink)));
...
TEST_CASE("player service destroy without unlink is safe", "[esp_player_service]")
{
...
TEST_CASE("player service feed path can re-feed after stop", "[esp_player_service]")
{
    audio_ut_capture_ctx_t cap = {0};
    esp_player_service_cfg_t cfg = ESP_PLAYER_SERVICE_CFG_DEFAULT();
    cfg.max_stream_num = 1;
...
TEST_CASE("player service url path state guards and event wiring", "[esp_player_service]")
{
...
TEST_CASE("player service ID3 API binding and error semantics", "[esp_player_service]")
{
    audio_ut_capture_ctx_t cap = {0};
    esp_player_service_cfg_t cfg = ESP_PLAYER_SERVICE_CFG_DEFAULT();
    cfg.max_stream_num = 1;
    ...
    esp_media_stream_id_t stream = ESP_MEDIA_DEFAULT_STREAM;
    const esp_extractor_id3_info_t *id3 = (const esp_extractor_id3_info_t *)0x1;

    /* No player yet: preference is stored; get must report INVALID_STATE. */
    TEST_ASSERT_EQUAL(ESP_OK, esp_player_service_enable_id3_parse(service, stream, true));
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_STATE, esp_player_service_get_id3_info(service, stream, &id3));
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG, esp_player_service_get_id3_info(service, stream, NULL));
    TEST_ASSERT_EQUAL(ESP_ERR_INVALID_ARG,
                      esp_player_service_enable_id3_parse(service, (esp_media_stream_id_t)99, true));
    ...
    TEST_ASSERT_EQUAL(ESP_OK, audio_ut_setup_custom(service, &sample_info,
                                                    audio_ut_writer, &cap));
    TEST_ASSERT_EQUAL(ESP_OK, esp_player_service_set_url(service, stream, "file:///tmp/ut_missing.mp3"));

    TEST_ASSERT_EQUAL(ESP_OK, esp_player_service_enable_id3_parse(service, stream, false));
    TEST_ASSERT_EQUAL(ESP_OK, esp_player_service_enable_id3_parse(service, stream, true));

    id3 = NULL;
    esp_err_t ret = esp_player_service_get_id3_info(service, stream, &id3);
    TEST_ASSERT_NOT_EQUAL(ESP_OK, ret);
    TEST_ASSERT_TRUE(ret == ESP_ERR_NOT_SUPPORTED || ret == ESP_FAIL || ret == ESP_ERR_INVALID_STATE);

    audio_ut_destroy_service(service);
}

TEST_CASE("player service mix cfg rejects DROP on URL source", "[esp_player_service]")
{
    audio_ut_capture_ctx_t cap = {0};
    esp_player_service_cfg_t cfg = ESP_PLAYER_SERVICE_CFG_DEFAULT();
    cfg.max_stream_num = 1;
...
TEST_CASE("player service auto-advances playlist on finish", "[esp_player_service]")
{
