...
static const char *TAG = "VPS_MCP";

extern const uint8_t esp_video_player_service_mcp_json_start[] asm("_binary_esp_video_player_service_mcp_json_start");
...
static bool parse_track_type(const char *s, esp_player_track_type_t *out)
{
    if (s == NULL || out == NULL) {
        return false;
    }
    if (strcmp(s, "audio") == 0) {
        *out = ESP_PLAYER_TRACK_TYPE_AUDIO;
        return true;
    }
    if (strcmp(s, "video") == 0) {
        *out = ESP_PLAYER_TRACK_TYPE_VIDEO;
        return true;
    }
    return false;
}

static esp_err_t tool_get_track_info(esp_player_service_t *svc, const char *args, char *result,
                                     size_t result_size)
{
    cJSON *root = NULL;
    esp_err_t ret = parse_args_object(args, &root);
    ...
    int index = 0;
    if (!json_get_stream(root, &stream) || !json_get_string(root, "type", &type_str)) {
        cJSON_Delete(root);
        return ESP_ERR_INVALID_ARG;
    }
    (void)json_get_int(root, "index", &index);
    esp_player_track_type_t type = ESP_PLAYER_TRACK_TYPE_VIDEO;
    if (!parse_track_type(type_str, &type) || index < 0) {
        cJSON_Delete(root);
        return ESP_ERR_INVALID_ARG;
    }
    cJSON_Delete(root);
    ...
    ret = esp_video_player_service_get_track_num(svc, stream, type, &track_num);
    if (ret != ESP_OK) {
        return write_simple_result(result, result_size, "get_track_info", ret);
    }
    esp_player_track_info_t info = {0};
    ret = esp_video_player_service_get_track_info(svc, stream, type, (uint16_t)index, &info);
    if (ret != ESP_OK) {
        return write_simple_result(result, result_size, "get_track_info", ret);
    }
    cJSON *out = cJSON_CreateObject();
    ...
    bool ok = cJSON_AddStringToObject(out, "operation", "get_track_info") &&
              cJSON_AddBoolToObject(out, "ok", true) &&
              cJSON_AddNumberToObject(out, "stream", stream) &&
              cJSON_AddNumberToObject(out, "track_num", track_num) &&
              cJSON_AddNumberToObject(out, "index", index);
    char fourcc[5];
    if (ok && type == ESP_PLAYER_TRACK_TYPE_AUDIO) {
        gmf_fourcc_to_str(info.audio_info.format, fourcc);
        ok = cJSON_AddStringToObject(out, "type", "audio") &&
             cJSON_AddStringToObject(out, "format", fourcc) &&
             cJSON_AddNumberToObject(out, "sample_rate", info.audio_info.sample_rate) &&
             cJSON_AddNumberToObject(out, "channels", info.audio_info.channels) &&
             cJSON_AddNumberToObject(out, "bits_per_sample", info.audio_info.bits_per_sample) &&
             cJSON_AddNumberToObject(out, "bitrate", info.audio_info.bitrate);
    } else if (ok) {
        gmf_fourcc_to_str(info.video_info.format, fourcc);
        ok = cJSON_AddStringToObject(out, "type", "video") &&
             cJSON_AddStringToObject(out, "format", fourcc) &&
             cJSON_AddNumberToObject(out, "width", info.video_info.width) &&
             cJSON_AddNumberToObject(out, "height", info.video_info.height) &&
             cJSON_AddNumberToObject(out, "fps", info.video_info.fps) &&
             cJSON_AddNumberToObject(out, "bitrate", info.video_info.bitrate);
    }
    esp_err_t json_ret = ok ? write_json_result(out, result, result_size) : ESP_ERR_NO_MEM;
    cJSON_Delete(out);
    return json_ret;
}

static esp_err_t tool_enable_track(esp_player_service_t *svc, const char *args, char *result,
                                   size_t result_size)
{
    cJSON *root = NULL;
    esp_err_t ret = parse_args_object(args, &root);
    ...
    bool enable = true;
    if (!json_get_stream(root, &stream) || !json_get_string(root, "type", &type_str) ||
        !json_get_int(root, "index", &index)) {
        cJSON_Delete(root);
        return ESP_ERR_INVALID_ARG;
    }
    (void)json_get_bool(root, "enable", &enable);
    esp_player_track_type_t type = ESP_PLAYER_TRACK_TYPE_VIDEO;
    if (!parse_track_type(type_str, &type) || index < 0) {
        cJSON_Delete(root);
        return ESP_ERR_INVALID_ARG;
    ...
    cJSON_Delete(root);
    ret = esp_video_player_service_enable_track(svc, stream, type, (uint16_t)index, enable);
    return write_simple_result(result, result_size, "enable_track", ret);
}
...
esp_err_t esp_video_player_service_mcp_schema_get(const char **out_schema)
{
    ...
    *out_schema = (const char *)esp_video_player_service_mcp_json_start;
    return ESP_OK;
}
                                               ...
                                               const char *args, char *result, size_t result_size)
{
    ...
    if (strcmp(tool->name, "esp_video_player_service_get_track_info") == 0) {
        return tool_get_track_info(svc, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_video_player_service_enable_track") == 0) {
        return tool_enable_track(svc, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_video_player_service_get_status") == 0) {
        return tool_get_status(svc, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_video_player_service_set_output_volume") == 0) {
        return tool_set_output_volume(svc, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_video_player_service_get_output_volume") == 0) {
        return tool_get_output_volume(svc, result, result_size);
    }
    if (strcmp(tool->name, "esp_video_player_service_set_volume") == 0) {
        return tool_set_volume(svc, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_video_player_service_get_volume") == 0) {
        return tool_get_volume(svc, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_video_player_service_set_url") == 0) {
        return tool_set_url(svc, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_video_player_service_play") == 0) {
        return tool_stream_op(svc, args, result, result_size, "play", esp_player_service_play);
    }
    if (strcmp(tool->name, "esp_video_player_service_pause") == 0) {
        return tool_stream_op(svc, args, result, result_size, "pause", esp_player_service_pause);
    }
    if (strcmp(tool->name, "esp_video_player_service_resume") == 0) {
        return tool_stream_op(svc, args, result, result_size, "resume", esp_player_service_resume);
    }
    if (strcmp(tool->name, "esp_video_player_service_stop") == 0) {
        return tool_stream_op(svc, args, result, result_size, "stop", esp_player_service_stop);
    }
    if (strcmp(tool->name, "esp_video_player_service_seek") == 0) {
        return tool_seek(svc, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_video_player_service_set_speed") == 0) {
        return tool_set_speed(svc, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_video_player_service_playlist_next") == 0) {
        return tool_stream_op(svc, args, result, result_size, "playlist_next", esp_player_service_next);
    }
    if (strcmp(tool->name, "esp_video_player_service_playlist_prev") == 0) {
        return tool_stream_op(svc, args, result, result_size, "playlist_prev", esp_player_service_prev);
    }
    if (strcmp(tool->name, "esp_video_player_service_playlist_play_index") == 0) {
        return tool_playlist_play_index(svc, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_video_player_service_set_repeat_mode") == 0) {
        return tool_set_repeat_mode(svc, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_video_player_service_set_mix_cfg") == 0) {
        return tool_set_mix_cfg(svc, args, result, result_size);
    }
    if (strcmp(tool->name, "esp_video_player_service_get_preempt_state") == 0) {
