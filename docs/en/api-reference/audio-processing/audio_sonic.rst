...
The adjustments of the first two parameters is represented by `float` values that provide the rate of adjustment. For example, to increase the speed of an audio sample by 2 times, call ``sonic_set_pitch_and_speed_info(el, 1.0, 2.0)``. To keep the speed as it is, call ``sonic_set_pitch_and_speed_info(el, 1.0, 1.0)``.
