...
Function Press Keys
    Two keys labeled *Rec* and *Mode*. They are routed to **ESP32-WROVER-E Module** and intended for developing and testing a UI for audio applications using dedicated API.
...
**MicroSD Card** and **Audio Codec Chip**

Two separate LDOs are provided for the MicroSD Card and the Audio Codec Chip. Both circuits have similar design that includes an inductor and double decoupling capacitors on both the input and output of the LDO.
...
An optional audio output is a pair of headphones fed from the same DAC as the amplifier IC.
