...
3. In ``Command Palette``, enter ``new project``, select the ``ESP-IDF: New Project`` option, and choose the ESP-IDF version to use. A window will appear with ESP-IDF and ESP-ADF (if you configured using install adf command or adding ADF_PATH in idf.customExtraVars setting) sections. Expand ESP-ADF and choose the desired example from the list of example projects.

4. Select an example, click ``Create project using example XX``, and later configure the new project, including the directory to save, the project name, and other settings.
