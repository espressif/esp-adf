...
#define RTMP_LCD_FRAME_BUFFER_NUM  (2)
...
#define RTMP_WIFI_ATTEMPTS          3
#define RTMP_WIFI_ATTEMPT_WAIT_SEC  10

static const char *TAG = "RTMP_EXAMPLE";
...
static void use_double_frame_buffer(void)
{
    ...
    if (strcmp(board_cfg->sub_type, ESP_BOARD_DEVICE_LCD_SUB_TYPE_DSI) == 0) {
        lcd_cfg.sub_cfg.dsi.dpi_config.num_fbs = RTMP_LCD_FRAME_BUFFER_NUM;
    }
    ...
    if (strcmp(board_cfg->sub_type, ESP_BOARD_DEVICE_LCD_SUB_TYPE_RGB) == 0) {
        lcd_cfg.sub_cfg.rgb.panel_config.num_fbs = RTMP_LCD_FRAME_BUFFER_NUM;
    }
...
static void apply_mic_gain(void)
{
    ...
    if (esp_codec_dev_set_in_gain(adc->codec_dev, RTMP_MIC_GAIN) != ESP_CODEC_DEV_OK) {
        ESP_LOGW(TAG, "Failed to set mic gain to %.1f dB", RTMP_MIC_GAIN);
    }
}

/* Every device is optional: a board without a camera can still run the server
   and pull, and a board without a display can still capture and push. */
static void init_board(void)
{
    ...
    if (cam_ret != ESP_OK) {
        ESP_LOGW(TAG, "Camera unavailable (%s); the push role runs audio only",
...
/* Encoders feed the push role, decoders feed the pull role */
...
esp_err_t rtmp_example_wifi_connect(const char *ssid, const char *password)
{
    ...
    for (int attempt = 1; attempt <= RTMP_WIFI_ATTEMPTS; attempt++) {
        ESP_LOGI(TAG, "Connecting to Wi-Fi SSID:%s (attempt %d/%d)", ssid_buf, attempt, RTMP_WIFI_ATTEMPTS);
        ret = esp_wifi_service_request_connect(s_wifi_service, ssid_buf, pass_buf, 10, RTMP_WIFI_ATTEMPT_WAIT_SEC);
        if (ret == ESP_OK) {
            ESP_LOGI(TAG, "Wi-Fi connected");
            return ESP_OK;
        }
        ESP_LOGW(TAG, "Wi-Fi attempt %d/%d failed: %s", attempt, RTMP_WIFI_ATTEMPTS, esp_err_to_name(ret));
    }

    ESP_LOGW(TAG, "Wi-Fi connect failed (%s); RTMP sessions will not reach the network. "
                  "Reconnect at any time with 'wifi <ssid> <password>'", esp_err_to_name(ret));
    return ret;
}

void rtmp_example_fill_ip(char *buf, size_t buf_len)
{
...
void app_main(void)
{
    ...
    (void)rtmp_example_wifi_connect(RTMP_WIFI_SSID, RTMP_WIFI_PASSWORD);

    ESP_LOGI(TAG, "[ 5 ] Start CLI");
    ESP_ERROR_CHECK(rtmp_cli_start());
