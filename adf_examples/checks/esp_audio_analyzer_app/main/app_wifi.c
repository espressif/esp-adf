...
static void wifi_init_softap(esp_netif_t *netif)
{
    ...
    snprintf((char *)wifi_config.ap.ssid, 32, "%s_%02X%02X%02X", EXAMPLE_ESP_WIFI_AP_SSID, mac[3], mac[4], mac[5]);
...
esp_err_t app_wifi_main(void)
{
    ...
    if (mode == WIFI_MODE_NULL) {
        ESP_LOGE(TAG, "WiFi mode could not be determined.");
    ...
    if (mode & WIFI_MODE_STA) {
        ...
        if (bits & WIFI_FAIL_BIT) {
            ESP_LOGE(TAG, "WiFi STA connect failed after max retries");
            ret = ESP_FAIL;
        } else if ((bits & WIFI_CONNECTED_BIT) == 0) {
            ESP_LOGE(TAG, "WiFi STA wait ended without connection");
            ret = ESP_FAIL;
        }
    }
